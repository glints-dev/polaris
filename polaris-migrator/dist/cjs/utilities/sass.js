...
const polarisCustomPropertyRegEx = new RegExp(polarisTokens.getCustomPropertyNames(polarisTokens.tokens).join('|'));
...
function getFunctionArgs(node) {
  ...
  node.nodes.forEach(node => {
    ...
    arg += valueParser__default["default"].stringify(node);
  });
...
function toTransformablePx(value) {
  const dimension = valueParser__default["default"].unit(value);
  if (!isTransformableLength(dimension)) return;
  return polarisTokens.toPx(`${dimension.number}${dimension.unit}`);
}
/**
...
function createInlineComment(text) {
  const comment = postcss__default["default"].comment({
...
function convertStylelintRuleToPostcssProcessor(ruleFn) {
  return (fileInfo, _, options) => {
    ...
    return postcss__default["default"](plugin).process(fileInfo.source, {
  ...
  const wrappedRule = (primary, secondaryOptions, context) => {
    ...
    const flushReportsAsComments = () => {
      ...
      for (const [node, reportsForNode] of reports) {
        node.before(createInlineComment(constants.POLARIS_MIGRATOR_COMMENT));
...
function setNodeValue(node, value) {
  ...
  const parsedValue = valueParser__default["default"](value).nodes[0];
...
exports.StopWalkingFunctionNodes = StopWalkingFunctionNodes;
exports.createInlineComment = createInlineComment;
exports.createSassMigrator = createSassMigrator;
exports.getFunctionArgs = getFunctionArgs;
exports.getNamespacePattern = getNamespacePattern;
exports.hasNegativeSassInterpolation = hasNegativeSassInterpolation;
exports.hasNumericOperator = hasNumericOperator;
exports.hasSassFunction = hasSassFunction;
exports.hasSassInterpolation = hasSassInterpolation;
exports.isNumericOperator = isNumericOperator;
exports.isPolarisVar = isPolarisVar;
exports.isSassFunction = isSassFunction;
exports.isSassVariable = isSassVariable;
exports.isTransformableDuration = isTransformableDuration;
exports.isTransformableLength = isTransformableLength;
exports.isUnitlessZero = isUnitlessZero;
exports.namespace = namespace;
exports.removeSassInterpolation = removeSassInterpolation;
exports.replaceNegativeSassInterpolation = replaceNegativeSassInterpolation;
exports.setNodeValue = setNodeValue;
exports.stripQuotes = stripQuotes;
exports.toTransformablePx = toTransformablePx;
exports.transformableDurationUnits = transformableDurationUnits;
exports.transformableLengthUnits = transformableLengthUnits;
