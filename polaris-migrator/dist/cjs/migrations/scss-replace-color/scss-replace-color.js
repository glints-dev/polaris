...
function scssReplaceColor(file, _, options) {
...
const plugin = (options = {}) => {
  const namespacedColor = sass.namespace('color', options);
  return {
    postcssPlugin: 'scss-replace-color',

    Declaration(decl) {
      // @ts-expect-error - Skip if processed so we don't process it again
      if (decl[processed]) return;
      if (!typeGuards.isKeyOf(propertyMaps, decl.prop)) return;
      ...
      parsed.walk(node => {
        ...
        if (node.value === 'var') {
          ...
          const polarisCustomPropertyIndex = args.findIndex(arg => polarisCustomPropertyRegEx.test(arg));
          const colorFnFallbackIndex = args.findIndex(arg => arg.startsWith(namespacedColor));
        ...
        if (node.value === namespacedColor) {
          var _colorFnArgs$, _colorFnArgs$2, _node$nodes$0$sourceI, _node$nodes$;

          const colorFnArgs = sass.getFunctionArgs(node).map(sass.stripQuotes);
          const hue = (_colorFnArgs$ = colorFnArgs[0]) !== null && _colorFnArgs$ !== void 0 ? _colorFnArgs$ : '';
          const value = (_colorFnArgs$2 = colorFnArgs[1]) !== null && _colorFnArgs$2 !== void 0 ? _colorFnArgs$2 : 'base';
          const forBackground = colorFnArgs[2]; // Skip color() with for-background argument

          if (forBackground) return; // Skip if we don't have a color for the hue and value

          if (!(typeGuards.isKeyOf(replacementMap, hue) && typeGuards.isKeyOf(replacementMap[hue], value))) return;
          const colorCustomProperty = replacementMap[hue][value];
          node.value = 'var';
          node.nodes = [{
            type: 'word',
      ...
      decl.value = parsed.toString(); // @ts-expect-error - Mark the declaration as processed
...
const polarisCustomPropertyRegEx = new RegExp(Object.keys(polarisTokens.colors).map(polarisTokens.createVar).join('|'));

module.exports = scssReplaceColor;
