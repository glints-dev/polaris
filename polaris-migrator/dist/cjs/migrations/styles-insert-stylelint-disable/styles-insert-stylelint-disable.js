...
async function stylesInsertStylelintDisable(file, _, options) {
  ...
  return postcss__default["default"]([stylelint__default["default"]({
    config: {
...
function createCommentNode(text) {
  return new postcss__default["default"].Comment({
    text,
    raws: {
      left: ' ',
      right: '',
...
function deleteExtraNewlinesBeforeNode(node) {
  node.raws.before = `${os__default["default"].EOL}${node.raws.before.replace(new RegExp(crossPlatformNewlineRegExp, 'g'), '')}`;
}
/**
...
module.exports = stylesInsertStylelintDisable;
