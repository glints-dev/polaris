...
const plugin = (options = {}) => {
  const namespacedBorderRadius = sass.namespace('border-radius', options);
  return {
      ...
      function handleBorderRadiusProps() {
        parsedValue.walk(node => {
          ...
          if (node.type === 'function') {
            if (sass.isSassFunction(namespacedBorderRadius, node)) {
              ...
              if (!typeGuards.isKeyOf(borderRadiusFunctionMap, value)) return;
              node.value = 'var';
              node.nodes = [{
                type: 'word',
...
module.exports = scssReplaceBorderRadius;
