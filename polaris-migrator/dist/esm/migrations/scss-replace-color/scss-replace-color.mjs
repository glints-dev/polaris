...
import { colors, createVar } from '@shopify/polaris-tokens';
import { StopWalkingFunctionNodes, getFunctionArgs, stripQuotes, namespace } from '../../utilities/sass.mjs';
    ...
    Declaration(decl) {
      ...
      parsed.walk(node => {
        ...
        if (node.value === namespacedColor) {
          ...
          if (!(isKeyOf(replacementMap, hue) && isKeyOf(replacementMap[hue], value))) return;
...
const polarisCustomPropertyRegEx = new RegExp(Object.keys(colors).map(createVar).join('|'));

export { scssReplaceColor as default };
